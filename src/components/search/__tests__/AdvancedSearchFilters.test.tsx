...
describe('AdvancedSearchFilters', () => {
  ...
  describe('Discussion Filters', () => {
    ...
    it('shows active filters count', () => {
      ...
      render(<AdvancedSearchFilters {...defaultProps} filters={filtersWithData} />);
      ...
      await waitFor(() => {
        ...
        const startDateInput = dateInputs.find(
          input =>
            ...
            input
              .closest('.advanced-search-filters__date-input-group')
              ?.querySelector('.advanced-search-filters__date-label')?.textContent === '開始日'
        );
        if (startDateInput) {
    ...
    it('handles participant count range', async () => {
      ...
      await waitFor(() => {
        ...
        const minParticipantsInput = numberInputs.find(
          input =>
            input.getAttribute('type') === 'number' &&
            input
              .closest('.advanced-search-filters__number-input-group')
              ?.querySelector('.advanced-search-filters__number-label')?.textContent === '最小' &&
            input
              .closest('.advanced-search-filters__section')
              ?.querySelector('.advanced-search-filters__section-title')?.textContent === '参加者数'
        );
