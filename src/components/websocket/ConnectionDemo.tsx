...
export const ConnectionDemo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleDisconnected = (message: any) => {
      setMessages(prev => [...prev, `❌ Disconnected: ${message.data?.reason || 'Unknown'} at ${new Date().toLocaleTimeString()}`]);
    };

    const handleError = (message: any) => {
      setMessages(prev => [...prev, `⚠️ Error: ${message.data?.error} at ${new Date().toLocaleTimeString()}`]);
    };
    ...
    const handleOffline = () => {
      setMessages(prev => [...prev, `📵 Device went offline at ${new Date().toLocaleTimeString()}`]);
    };

    const handleReconnectScheduled = (message: any) => {
      setMessages(prev => [...prev, `🔄 Reconnect attempt ${message.data?.attempt} scheduled for ${message.data?.delay}ms at ${new Date().toLocaleTimeString()}`]);
    };

    const handleSyncComplete = (message: any) => {
      setMessages(prev => [...prev, `🔄 Sync completed: ${message.data?.messageCount || 0} messages at ${new Date().toLocaleTimeString()}`]);
    };
  ...
  const handleConnect = async () => {
    try {
      await connect();
      setMessages(prev => [...prev, `🔌 Manual connect requested at ${new Date().toLocaleTimeString()}`]);
    } catch (error) {
      setMessages(prev => [...prev, `❌ Manual connect failed: ${error} at ${new Date().toLocaleTimeString()}`]);
    }
  };

  const handleDisconnect = () => {
    disconnect();
    setMessages(prev => [...prev, `🔌 Manual disconnect requested at ${new Date().toLocaleTimeString()}`]);
  };
  ...
  const simulateOffline = () => {
    ...
    setMessages(prev => [...prev, `📵 Simulated offline mode at ${new Date().toLocaleTimeString()}`]);
    
    // Automatically come back online after 5 seconds
    setTimeout(() => {
      setIsSimulatingOffline(false);
      window.dispatchEvent(new Event('online'));
      setMessages(prev => [...prev, `🌐 Simulated online mode at ${new Date().toLocaleTimeString()}`]);
    }, 5000);
  };
  ...
  const handleClearErrors = () => {
    clearErrors();
    setMessages(prev => [...prev, `🧹 Cleared connection errors at ${new Date().toLocaleTimeString()}`]);
  };
  ...
  return (
    <div className="connection-demo">
      <div className="connection-demo__header">
        ...
        <p>This demo showcases the enhanced connection state management, offline detection, and error handling features.</p>
      </div>
      ...
      <div className="connection-demo__info">
        <div className="connection-demo__info-grid">
          <div className="connection-demo__info-item">
            ...
            <span className={`connection-demo__state connection-demo__state--${connectionState.toLowerCase()}`}>
              {connectionState}
            </span>
          </div>
          ...
          <div className="connection-demo__info-item">
            ...
            <span>{lastSyncTimestamp ? new Date(lastSyncTimestamp).toLocaleString() : 'Never'}</span>
          </div>
        </div>
      ...
      <div className="connection-demo__events">
        ...
        <div className="connection-demo__events-list">
          {messages.length === 0 ? (
            <p className="connection-demo__no-events">No events yet. Try connecting or simulating offline mode.</p>
          ) : (
      ...
      <div className="connection-demo__features">
        ...
        <ul className="connection-demo__features-list">
          <li>
            <strong>Offline Detection:</strong> Automatically detects when the device goes offline/online using the Navigator API
          </li>
          <li>
            <strong>Auto-Reconnection:</strong> Automatically attempts to reconnect with exponential backoff and jitter
          </li>
          <li>
            <strong>Connection State Management:</strong> Tracks detailed connection states including OFFLINE, CONNECTING, CONNECTED, etc.
          </li>
          <li>
            <strong>Error History:</strong> Maintains a history of connection errors for debugging
          </li>
          <li>
            <strong>Sync on Reconnect:</strong> Requests missed messages when reconnecting after a disconnection
          </li>
          <li>
            <strong>Heartbeat Monitoring:</strong> Sends periodic ping messages to detect connection health
          </li>
          <li>
            <strong>Graceful Degradation:</strong> Handles various disconnection scenarios appropriately
          </li>
        </ul>
      </div>
    </div>
